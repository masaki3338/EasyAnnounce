...
export type ScreenType =
  ...
  | "defense"
  | "gather"          // ← ここに追加
  | "gameGreeting"    // ← 今後追加予定の画面名例
  | "seatIntroduction"; // ← 今後追加予定の画面名例
...
const PreGameAnnouncement: React.FC<Props> = ({ onNavigate, onBack }) => {
  return (
    ...
    <button
      onClick={() => onNavigate("gameGreeting")} // こちらは後で画面を作るなら "gameGreeting" などにしてください
      className="w-full bg-green-600 text-white px-4 py-3 rounded-lg"
    >
      試合開始挨拶（先攻チームアナウンス）
    </button>
    ...
    <button
      onClick={() => onNavigate("seatIntroduction")} // こちらも画面を作れば "seatIntroduction" などに
      className="w-full bg-green-600 text-white px-4 py-3 rounded-lg"
    >
      シート紹介
    </button>
