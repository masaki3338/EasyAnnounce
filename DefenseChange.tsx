...
const generateAnnouncementText = (
  ...
  battingOrder.forEach((entry, index) => {
    if (entry.reason === "代打" || entry.reason === "代走") {
      ...
      if (!isReplaced && currentId === entry.id && currentId === initialId) {
        result.push(`先ほど${entry.reason}致しました ${player.lastName}${honorific} がそのまま ${posName} に入ります。`);
        result.push(`${index + 1}番 ${posName} ${player.lastName}${honorific} 。`);

        // replace/mixed/shift はこのあと通常処理を無効化したい場合のみ
        replace.length = 0;
        mixed.length = 0;
        shift.length = 0;
      }
      // ✅ 分岐2: 守備位置が変わって入った（交代なし＋違う守備位置）
        ...
        result.push(`先ほど${entry.reason}致しました ${player.lastName}${honorific} が ${posName} に入り、`);

        if (shiftRecord && shiftPlayer) {
          const shiftH = shiftPlayer.isFemale ? "さん" : "くん";
          result.push(`${posName}の ${shiftPlayer.lastName}${shiftH} が ${shiftToPosName} に入ります。`);

          // shiftリストから除外して二重表現防止
        ...
        // 交代で入った選手の打順
        announceOrderList.push(`${index + 1}番 ${posName} ${player.lastName}${honorific}`);
        ...
        if (shiftRecord && shiftPlayer && shiftToPos) {
          ...
          const shiftH = shiftPlayer.isFemale ? "さん" : "くん";
          if (shiftOrder !== -1) {
            announceOrderList.push(`${shiftOrder + 1}番 ${shiftToPosJP} ${shiftPlayer.lastName}${shiftH}`);
          }
        }
        // 🔽 打順順にソートして追加
        announceOrderList
  ...
  allReplacements.forEach((r, index) => {
    if (r.type === "mixed") {
      ...
      if (ChangeFlg === 2) {  //代打時の表示
          result.push(
          `先ほど代打致しました ${r.from.lastName}${fromH} に代わりまして、${r.to.lastName}${toH} が入り${posJP[r.toPos]}、`);      }
      else{  
...
setBenchPlayers((prev) => prev.filter((p) => p.id !== playerId));
  ...
  battingOrder.forEach((starter, index) => {
    const starterInitialPos = getPositionName(initialAssignments, starter.id);
    const starterAssignedId = newAssignments[starterInitialPos];

    if (starterAssignedId === starter.id) {
      setBattingReplacements((prev) => {
        const copy = { ...prev };
        delete copy[index];
        return copy;
...
    >
      {player ? `${player.lastName ?? ""}${player.firstName ?? ""} #${player.number}` 
              : <span className="text-gray-300">空き</span>}
    </div>
  );
})}
</div>
        ...
        return {
          ...
          jsx: (
            <li key={`pinch-assigned-${index}`}>
              代打 ➡ {currentPos}：{replaced.lastName}{replaced.firstName} #{replaced.number}
            </li>
          )
        };
      }
        ...
        return {
          ...
          jsx: (
            <li key={`replaced-${index}`}>
              {initialPos}：{starter.lastName}{starter.firstName} #{starter.number} ➡ {currentPos}：
...
<div className="text-red-700 bg-red-100 border border-red-400 text-lg font-bold whitespace-pre-line rounded px-4 py-3 mb-4 shadow-md leading-relaxed">
  {announcementText}
</div>
